...
티커를 입력하면 백테스트 차트와 거래 내역을 보여주는 모바일 친화 웹앱입니다.

- **Mac**: `run.command` 파일을 더블클릭 (또는 터미널에서 `./run.command`)
- **Windows**: `run.bat` 파일을 더블클릭

최초 실행 시 가상환경 생성과 패키지 설치로 1~2분 걸리고, 이후에는 바로 켜집니다. 실행되면 브라우저가 자동으로 `http://127.0.0.1:8000` 을 엽니다.

**같은 와이파이의 폰에서 보기**: 컴퓨터의 IP 주소를 확인한 뒤(Mac/Linux는 `ifconfig`, Windows는 `ipconfig`) 폰 브라우저에서 `http://<컴퓨터IP>:8000` 으로 접속하세요. 홈 화면에 추가하면 앱처럼 쓸 수 있습니다.
